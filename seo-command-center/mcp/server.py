...
import json, os, queue, threading, time, html
...
def _render_html(o) -> str:
    ...
    recs = "".join(f"<li>{html.escape(r)}</li>" for r in o.get("recommendations", []))

    # --- Fixes Section ---
    fixes_obj = o.get("fixes", {})
    titles_fixes = fixes_obj.get("titles", [])
    redirect_fixes = fixes_obj.get("redirect_map", [])

    fix_html = ""
    if titles_fixes or redirect_fixes:
        meta_html = ""
        if titles_fixes:
            meta_rows = "".join([f"<tr><td>{html.escape(f['url'])}</td><td>{html.escape(f['old'])}</td><td>{html.escape(f['new'])}</td></tr>" for f in titles_fixes])
            meta_html = f'<h4 style="margin-bottom:10px; color:#c8c5be;">Metadata Rewrites</h4><table><thead><tr><th>URL</th><th>Original</th><th>Suggested</th></tr></thead><tbody>{meta_rows}</tbody></table>'

        red_html = ""
        if redirect_fixes:
            red_rows = "".join([f"<tr><td>{html.escape(f['from'])}</td><td>{html.escape(f['to'])}</td><td>{html.escape(f['reason'])}</td></tr>" for f in redirect_fixes])
            red_html = f'<h4 style="margin-bottom:10px; color:#c8c5be;">Redirect Suggestions</h4><table><thead><tr><th>From</th><th>To</th><th>Reason</th></tr></thead><tbody>{red_rows}</tbody></table>'

        fix_html = f'<div class="card"><h3 style="margin-top:0">Generated Fixes</h3>{meta_html}{red_html}</div>'
...
<div class="card"><h3 style="margin-top:0">Issues (prioritized)</h3><table><thead><tr><th>Severity</th><th>Issue</th><th>URLs</th><th>What it means</th></tr></thead>
<tbody>{rows or '<tr><td colspan=4 class=muted>No issues detected.</td></tr>'}</tbody></table></div>
<div class="card"><h3 style="margin-top:0">Recommendations</h3><ul>{recs or '<li class=muted>None generated.</li>'}</ul></div>
{fix_html}
